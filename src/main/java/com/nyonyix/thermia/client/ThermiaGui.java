...
public class ThermiaGui
{
    ...
    {
        event.registerAbove(VanillaGuiLayers.ARMOR_LEVEL, ResourceLocation.fromNamespaceAndPath(Thermia.MODID, "player_temp"), ThermiaGui::renderPlayerTemp);
    }
    ...
    private static void renderPlayerTemp(GuiGraphics graphics, DeltaTracker delta)
    {
        ...
        int y = graphics.guiHeight() - gui.rightHeight - (WIDGET_SIZE / 2) - 1;
        ...
        float environmentNormalised = Mth.clamp((environmentTemperature + 30f) / 60f, 0f, 1f);
        ...
        graphics.blit(TEXTURE, 0, 0, WIDGET_SIZE, WIDGET_SIZE,16, 0, 32, 32, 80, 32);

        RenderSystem.setShaderColor(envR, envG, envB, 1.0f);
        graphics.blit(TEXTURE,0, 0, WIDGET_SIZE, WIDGET_SIZE, 48, 0, 32, 32, 80, 32);
